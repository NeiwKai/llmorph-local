...
# Common functional words to exclude when filtering or trimming boundary noise.
# ----------------------------------------------------
STOPWORDS = {
...
def extract_bigram_context_transformations(tokens1: List[str], tokens2: List[str]) -> List[Tuple[str, str]]:
    """
    Extracts 2-Gram phrase pairs where context changes.
    Trims boundary stopwords while keeping internal 2-gram content word pairs intact.
    """
    ...
    for tag, i1, i2, j1, j2 in matcher.get_opcodes():
        if tag in ['replace', 'delete', 'insert']:
            # Expand context window slightly to capture 2-gram context
            start_i = max(0, i1 - (1 if i2 - i1 < 2 and i1 > 0 else 0))
            end_i = min(len(tokens1), i2 + (1 if (i2 - start_i) < 2 else 0))

            start_j = max(0, j1 - (1 if j2 - j1 < 2 and j1 > 0 else 0))
            end_j = min(len(tokens2), j2 + (1 if (j2 - start_j) < 2 else 0))
            ...
            # Ensure valid non-empty context change
...
def plot_metric_table(metric_list: List[dict], mode: str, threshold: float, precision: float, task_name: str, date_time_str: str):
    """
    Renders and saves a high-quality table image displaying 2-Gram evaluation metrics,
    ...
    title_text = f"2-Gram Context Transformation Metrics Table ({mode} Mode)\n" \
    ...
    plot_filename = f"{task_name}_{date_time_str}_2gram_table.png"
...
def main():
    ...
    # Extract 2-Gram context transformation pairs
    aligned_pairs = extract_bigram_context_transformations(tokens1, tokens2)
    ...
    # Generate dense vector embeddings for 2-gram phrases
    ...
    print(f"\nTarget Mode: {mode}")
    print(f"Valid 2-Gram Changes: {valid_changes} / {len(metric_list)}")
